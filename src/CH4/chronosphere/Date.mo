// src: https://github.com/enzoh/chronosphere/blob/fa8e8b1d21/src/Date.mo
...
module Date {
  ...
  public func hash(date: Date) : Hash.Hash {
      let #Date d = date;
      Int.hash(Int32.toInt(d))
  };

  public func equal(d1: Date, d2: Date) : Bool {
      d1 == d2
  };

  // da1 <= da2
  public func iter(da1: Date, da2: Date) : Iter.Iter<Date> {
      let #Date d1 = da1;
      let #Date d2 = da2;
      let iter = range(d1, d2);
      Iter.map(iter, func (x: Int32) : Date {  #Date(x)  })
  };

  // 0 <= x <= y
  public class range(x : Int32, y : Int32) {
    var i = x;
    public func next() : ?Int32 { if (i > y) { null } else {let j = i; i += 1; ?j} };
  };
  ...
  /**
   * Get the Timestamp to Data
   */
  public func stamp(time: Int) : Date {
    let base = Int32.fromInt(time / 86400_000_000_000);
    #Date base
  };

  public func dateVal(date: Date) : Int32 {
      let #Date base = date;
      base
  };

  // to the date last sec
  public func toSec(date: Date) : Int {
      let d = dateVal(date);
      Int32.toInt((d+1)*86400 - 1)
  };
